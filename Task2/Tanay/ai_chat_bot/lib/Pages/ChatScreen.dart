...
class _ChatScreenState extends State<ChatScreen> {
 ...
 DateTime _initialDate = DateTime.now().subtract(const Duration(hours: 25));
  ...
  bool downbtn = false;
  bool showSend=false;
    final List _elements=[{'Date': DateFormat.yMMMd().format(DateTime.now().subtract(const Duration(hours: 48))), 'message': ChatMessage(
      ifUser: true,
      text: "Hello! Nice to meet you. (Demo)",
      time: "11:59 PM",
    )},{'Date': DateFormat.yMMMd().format(DateTime.now().subtract(const Duration(hours: 24))), 'message': ChatMessage(
      ifUser: false,
      text: "It's nice to meet you too!😊 How can i help you today? (Demo) ",
      time: "12:01 AM",
    )}];
    List<String> response_history=["Hi"];
  ...
  void _sendContents(Content content) async{
    try {
      ...
      var responses = await _chat.sendMessageStream(content);
      String resptext="";
      await for(final response in responses){
        ...
        setState(() {
          ...
          response_history[response_history.length-1]+=resptext;
        });
        _scrolldown();
      }
              ...
              children: [
                Container(
                  ...
                  child: GroupedListView<dynamic, String>(
                    ...
                    groupSeparatorBuilder: (String value)=>Container(height: 65,child: Datebox(date: value)),
